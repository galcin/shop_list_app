...
class WatchExpiringSoonUseCase {
  final IPantryRepository _repository;

  WatchExpiringSoonUseCase(this._repository);

  Stream<Either<Failure, List<PantryItem>>> call({int withinDays = 3}) {
    try {
      return _repository
          .watchExpiringSoon(days: withinDays)
          .map((items) => Right(items));
    } catch (e) {
